...
import { videosOptions, mapVideos } from '../models/videos'
...
const actions = {
    getVideos({commit}) {
        ...
        fs.collection('videos').orderBy('order').get().then((shots) => {
            let payload = [];
            shots.forEach((doc) => {
    ...
    setVideos({commit}) {
        commit('setLoading', true)
        fl.content.get('VideoMaterials', videosOptions).then( shot => {
            if(!shot) return;
            const payload = mapVideos(shot);
            commit('setLoading', false);
            payload.map((item) => {
                fs.collection('videos').doc('id'+item.id).set(item);
            })
        })
    }
}
